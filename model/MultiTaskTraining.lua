...
cmd:option('-K',7,'number of paths to consider')
...
--options added for multitasking
cmd:option('-typeBatchSize',0,'batchsize for entity type expts.')
cmd:option('-type_l2',0,'l2 for entity type expts.')
cmd:option('-data_dir_types','','train_directory for entity type expts.')
cmd:option('-typeNumEpoch',0,'numEpochs for entity type expts.')
cmd:option('-typeRegularize',0,'Regularize entity type prediction?')
cmd:option('-typeGradClip',0,'typeGradClip?')
cmd:option('-typeGradClipNorm',0,'typeGradClipNorm')
cmd:option('-typeLearningRate',0,'typeLearningRate')
...
local typeMiniBatch = params.typeBatchSize
...
if createExptDir then
	...
	configFile:write('typeMiniBatch\t'..params.typeBatchSize..'\n')
	configFile:write('type_l2\t'..params.type_l2..'\n')
	configFile:write('data_dir_types\t'..params.data_dir_types..'\n')
	configFile:write('typeNumEpoch\t'..params.typeNumEpoch..'\n')
	configFile:write('typeRegularize\t'..params.typeRegularize..'\n')
	configFile:write('typeGradClip\t'..params.typeGradClip..'\n')
	configFile:write('typeGradClipNorm\t'..params.typeGradClipNorm..'\n')
	configFile:write('typeLearningRate\t'..params.typeLearningRate..'\n')
end
...
-------------------------------Start of multitasking code.-------------------------------

local entityTypeLookupTable = FeatureEmbedding:getEntityTypeLookUpTable()
local colEncoder = nn.LookupTable(entityTypeVocabSize, entityTypeEmbeddingDim)
local params1, _ = training_net:parameters()
local rowEncoder = nn.LookupTable(entityVocabSize, entityEmbeddingDim)
local type_input_data = params.data_dir_types..'/train.torch'
local genNeg = true
local typeBatcher = TypeBatcher(type_input_data, params.typeBatchSize, shuffle, genNeg, entityVocabSize)
local tNet = TypeNetwork(useCuda)
--get the network
local typeNet = tNet:build_network(rowEncoder, colEncoder, entityTypeLookupTable)

print(typeNet)

--initialize
local typeParams, typeGradParams = typeNet:parameters()
for k, typeParams in ipairs(typeParams) do
    typeParams:uniform(-0.1, 0.1)
end

--criterion
local typeCriterion = nn.BPRLoss()
...
--colEncoder will share param with the entityTypeLookupTable.
colEncoder:share(entityTypeLookupTable,'weight', 'gradWeight', 'bias', 'gradBias')
...
local typeOptimMethod = nil
if(useAdam) then
	optimMethod = optim.adam
	typeOptimMethod = optim.adam
	print('Using Adam!')
	if createExptDir then	
	...
	typeOptConfig = {learningRate = params.typeLearningRate,beta1 = beta1,beta2 = beta2,epsilon = epsilon}
else
	print('Using adagrad!')
...
optInfo = {
	...
	typeOptimMethod = typeOptimMethod,
	optConfig = optConfig,
	typeOptConfig = typeOptConfig,
    optState = {},  
    typeOptState = {},
    ...
    l2 = params.l2,
    typeL2 = params.type_l2,
    createExptDir = createExptDir,
    regularize = params.regularize,
    typeRegularize = params.typeRegularize,
    typeGradClip = params.typeGradClip,
    typeGradClipNorm = params.typeGradClipNorm,
    saveFileNameTypes = params.exptDir..'/type_model-'
    -- recurrence = recur
}
--------Callbacks-------
callbacks = {}
...
local trainingOptions = {
    ...
    typeNumEpochs = params.typeNumEpoch,
    epochHooks = callbacks,
    minibatchsize = params.minibatch,
}
-----------------------------------	
params.learningRate = params.pretrainLearningRate
optimizer = MyOptimizerMultiTask(training_net, training_net, typeNet, criterion, typeCriterion, trainingOptions, optInfo, params.rnnType,colEncoder,entityTypeLookupTable) 
optimizer:train(trainBatcher, typeBatcher)