...
--pretrain with the type_prediction_model
print('Pretraining the type embeddings....')
local preTrainedTypeModelPath = '/iesl/local/rajarshi/expts/entity_type_baseline/model--15'
local preTrainedTypeModel = torch.load(preTrainedTypeModelPath)
local preTrainParams, preTrainGradParams = preTrainedTypeModel:parameters()

--get the entityLookupTable out of gpu
preTrainParams[1]:double()
local preTrainedTypeLookupTable = preTrainParams[2]
local embeddingLayerParams, _ = embeddingLayer:parameters()
local typeLookupTable = embeddingLayerParams[1] -- WARNING - this might change
assert(typeLookupTable:size(1) == preTrainedTypeLookupTable:size(1))
assert(typeLookupTable:size(2) == preTrainedTypeLookupTable:size(2))
typeLookupTable:copy(preTrainedTypeLookupTable)
